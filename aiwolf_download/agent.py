...
"""agent module."""
# 型アノテーションに自分自身のクラスや循環参照するクラスを指定したい場合に有用
from __future__ import annotations

import re
# Enum : 列挙型：定数として機能する値に名前をつけたのもの集まり
from enum import Enum
# ClassVar : クラス変数：インスタンスによらず共通の値を持つ変数
# Pattern : 正規表現のパターンオブジェクト
# Match : 正規表現のマッチングオブジェクトs
...
class Agent:
    ...
    @staticmethod # インスタンス化しなくても呼び出せるメソッド
    def compile(input: str) -> Agent:
        ...
        if m:
            if m.group(1) == "ANY":
                return Agent(0xff) # 0x:16進数
            else:
                return Agent(int(m.group(2)))
        return Agent(0)

    # インスタンス生成時に、クラスをカスタマイズするためのもの
    ...
    @property # 付与されたメソッド名を使って、読み出し専用属性のgetterとする 
