        ...
        for i in range(self.N):
            for j in range(self.N):
                ...
                if debug and abs(score_matrix.get_score(i, self.assignment[i], j, self.assignment[j])) >= 4.5:
                    Util.debug_print("score_matrix.get_score(", i+1, self.assignment[i], j+1, self.assignment[j], ")\t", "= ",round(score_matrix.get_score(i, self.assignment[i], j, self.assignment[j]), 2))
