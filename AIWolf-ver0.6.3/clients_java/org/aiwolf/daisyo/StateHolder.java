...
public class StateHolder {
	...
	void game_init(List<Integer> fixed, int me, int N, int role, Parameters params){
		...
		if(debug) System.out.println("GAMESTART TYUMOKU : " + me);
	}
	...
	void process(Parameters params, ArrayList<GameData> logs){
		...
		for(;head < logs.size();head++){
			...
			case EXECUTED:
			{
				gamestate.agents[g.object].isAlive = false;
				if(debug)
			...
			{
				if(g.white){
					if(gamestate.agents[g.object].isAlive){
			...
			{
				gamestate.gameCount++;
				if(debug) System.out.println("GAMEEND");
			}
			break;
			}
